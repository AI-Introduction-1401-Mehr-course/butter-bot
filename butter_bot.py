...
class ButterBotStateSpace(StateSpace):
    ...
    def _is_valid(self) -> bool:
        ...
        for cell in cells:
            if not(0 <= cell[0] < self.state.columns and 0 <= cell[1] < self.state.rows):
                return False
    ...
    def result(self, action: Action) -> Self:
        ...
        if new_bot_cell in self.state.butter_cells:
            ...
            new_butter_cells=list(self.state.butter_cells[:butter_cell_index] + [moved_butter_cell, ] + self.state.butter_cells[butter_cell_index+1:])

        return ButterBotStateSpace({**self.state._asdict(), 'bot_cell':new_bot_cell, 'butter_cells':new_butter_cells})
